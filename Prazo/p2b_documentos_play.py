...
from Fix.playwright_core import medir_tempo
...
def _encontrar_documento_relevante(page: Page) -> Tuple[Optional[Any], Optional[Any], int]:
    """
    ...
    try:
        from Fix.playwright_core import buscar_documentos_sequenciais, verificar_documento_decisao_sentenca
...
def _extrair_texto_documento(page: Page, doc_link: Any) -> Optional[str]:
    """
...
def _extrair_com_extrair_direto(page: Page) -> Optional[str]:
...
def _extrair_com_extrair_documento(page: Page) -> Optional[str]:
...
def _fechar_aba_processo(page: Page) -> None:
    """
...
def _processar_regras_gerais(page: Page, texto_normalizado: str, doc_idx: int = 0):
    """
